...
const KPICard: React.FC<{
  ...
  <div className="p-4 rounded-lg border border-border bg-card">
    <div className="flex items-start justify-between">
      ...
      {icon && (
        <div className="p-2 rounded-lg bg-[#7FFFC3]/10 text-[#7FFFC3]">
          {icon}
        </div>
      )}
    </div>
...
// Code preview panel
const CodePreview: React.FC<{ appName: string; components: string[]; systemPrompt: string }> = ({
  appName,
  components,
  systemPrompt,
}) => {
  ...
  const generatedCode = `// Generated by TamboStudio
import { TamboProvider } from "@tambo-ai/react";

// App: ${appName}
// Components: ${components.join(", ")}

export function ${appName.replace(/\s+/g, "")}() {
  return (
    <TamboProvider
      components={[
    ...
    <div className="border-t border-border">
      ...
      {showCode && (
        <div className="px-4 pb-4">
          <pre className="p-3 rounded-lg bg-[#1a1a1a] text-xs text-gray-300 overflow-x-auto font-mono">
...
export const LivePreview: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const { selectedTemplate, components, appName, systemPrompt, launchApp } = useStudioStore();
    ...
    return (
      <div className={cn("flex items-center justify-center h-full", className)}>
        <div className="text-center">
          <div className="w-16 h-16 rounded-2xl bg-[#7FFFC3]/10 flex items-center justify-center mx-auto mb-4">
            <Sparkles className="h-8 w-8 text-[#7FFFC3]/50" />
          </div>
          <p className="text-muted-foreground mb-2">Preview will appear here</p>
          <p className="text-xs text-muted-foreground/60">Describe your app to get started</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={cn("flex flex-col h-full", className)}>
      {/* Header */}
      <div className="flex items-center justify-between p-3 border-b border-border bg-gradient-to-r from-[#7FFFC3]/5 to-transparent">
        <div className="flex items-center gap-2">
          <div className="w-6 h-6 rounded-md bg-[#7FFFC3]/20 flex items-center justify-center">
            <Play className="h-3 w-3 text-[#7FFFC3]" />
          </div>
          <div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2 bg-[#7FFFC3] text-gray-900 rounded-lg text-sm font-semibold hover:bg-[#6ee6b0] transition-all hover:scale-105 disabled:opacity-50 disabled:hover:scale-100 shadow-lg shadow-[#7FFFC3]/20"
        >
          {isLaunching ? (
            <>
      ...
      <CodePreview 
        appName={appName}
        components={enabledComponents.map(c => c.name)}
